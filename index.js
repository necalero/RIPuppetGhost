...
async function login(page) {
  ...
  await page.fill('#identification', "nicolas.calero1@gmail.com");
  await page.fill('#password', "123456789*");

  // Click on the sign-in button
  await page.click('#ember5');

  // Wait for navigation or some indication that the login was successful
    ...
    if (location.x !== 0 && location.y !== 0 && location.width !== 0 && location.height !== 0){
      ...
      if(inputValuesFlag){
        ...
        if(index !== -1){
          ...
          let valueToType = inputValues[index]
          await page.keyboard.type(valueToType);
          console.log("Typed: "+valueToType+" in the field "+id)
        }
        else{
